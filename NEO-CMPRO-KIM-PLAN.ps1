param(
    ...
    [string]$PlayerName = "김민선7"
)
$ErrorActionPreference = "Stop"

Write-Host "[NEO CMPRO PLAYER PLAN] player=$PlayerName code=$PlayerCode"
Push-Location klpga_pipeline
try {
    foreach ($GameCode in $GameCodes) {
        Write-Host ""
        Write-Host "=== PLAYER PLAN $GameCode ==="
        py scripts/plan_cmpro_player.py --game $GameCode --player-code $PlayerCode --player-name "$PlayerName"

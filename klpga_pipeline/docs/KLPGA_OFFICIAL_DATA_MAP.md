...
| Player profile | birth year, nationality, team/sponsor, career starts/wins/earnings, season splits | PLAYER → 선수명 → 등급 → 소속 → 출생년도 → 회원번호 → 입회년도 *(field order as reported)* | — | — | — | — | `GET /web/profile/mainRecord?playerCode=<code>` | GET *(reported)* | text/html *(reported, page — not stated to be JSON)* | **REPORTED, NOT YET INDEPENDENTLY CONFIRMED** — URL, method, and field order/labels reported by the project owner via chat (not observed in a DevTools capture by this project); two example values were also reported (playerCode=9788 박혜준 → 두산건설 We's; playerCode=11134 서교림 → 삼천리) but have not been cross-checked against a real HTTP response captured by this project — this sandbox has no network access to klpga.co.kr. A fetch-only collector (`klpga.collectors.player_profile.fetch_player_profile_html`) and a first-contact diagnostic script (`scripts/53_fetch_player_profile_sample.py`) exist to capture a real fixture on a machine with real network access; no parser exists yet and no DOM structure is assumed. See `PLAYER_PROFILE_ENDPOINT` in `config.py` for the full provenance note. |
...
| Player | team/sponsor (소속) endpoint+field REPORTED (not yet independently confirmed) — see Table 1; fetch-only collector + diagnostic script exist, no parser yet | birth year/nationality (schema columns already exist, unconfirmed fill), career/season splits; and, for team/sponsor specifically, the real DOM structure of `/web/profile/mainRecord` (not yet captured) | 🟡 endpoint+field reported, not yet fetched by this project | UNKNOWN | P1 |

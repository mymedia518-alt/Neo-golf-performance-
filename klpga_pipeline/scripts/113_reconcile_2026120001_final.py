...
def main():
 ...
 for n in range(3):
  ...
  allrows += rows(b,n+1)
 codes=sorted({r['playerCode'] for r in allrows}); final={c:{'playerCode':c,'rounds':[r['round'] for r in allrows if r['playerCode']==c],'status':next((r['status'] for r in reversed(allrows) if r['playerCode']==c and r['status']), 'OTHER_OFFICIAL_STATUS')} for c in codes}
 counts={}

...
from klpga.discovery.record_fetch import (  # noqa: E402
    fetch_and_analyze as _shared_fetch_and_analyze,
    request_form,
    sanitize_identity_key_for_filename,
)
from klpga.discovery.request_log import to_log_csv, to_log_jsonl  # noqa: E402
...
def _request_form(leaf: SampledLeaf, season: str) -> dict:
    """Thin wrapper over klpga.discovery.record_fetch.request_form —
    extracted (Round 9 follow-up) so Phase B2's full-sweep script
    reuses this exact, already-tested request-shape logic instead of
    duplicating it."""
    return request_form(leaf, season)


def _sanitize_identity_key_for_filename(key: str) -> str:
    """Thin wrapper over klpga.discovery.record_fetch.
    sanitize_identity_key_for_filename — see `_request_form`'s
    docstring for why this was extracted."""
    return sanitize_identity_key_for_filename(key)


def fetch_and_analyze(
    client: PoliteHttpClient, leaf: SampledLeaf, season: str, *, tag: str = "?", raw_dir: Path | None = None
):
    """Thin wrapper over klpga.discovery.record_fetch.fetch_and_analyze
    (Round 9 follow-up extraction — see that module's docstring) that
    binds this script's own `_log` (which also updates `_LAST_MARKER`
    for the top-level KeyboardInterrupt handler) as the log sink.
    Behavior, including the raised RateLimitBlockedError, is otherwise
    unchanged from before this extraction."""
    return _shared_fetch_and_analyze(client, leaf, season, tag=tag, raw_dir=raw_dir, log=_log)

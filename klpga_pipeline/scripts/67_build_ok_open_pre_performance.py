...
def build(game_code: str | None = None):
    """Freeze the PRE performance snapshot for an explicit game_code
    (default: the operationally-active tournament, unchanged historical
    behavior). Entirely file/DB-based -- no live network dependency."""
    context = load_tournament_context(game_code)
    GAME = context.game_code
    CUT_DATE = date.fromisoformat(context.start_date)
    CUTOFF = f"{context.start_date}T00:00:00+09:00"
    entry_path = context.artifact_path("entry_snapshot")
    if not entry_path.exists():
        collect_entry_list_snapshot(context, cache_dir=ROOT / "data" / "raw_cache" / "http", db_path=DB)
    ...
    for e in entries:
        ...
        wins={"current":rs[-1:] ,"recent3":rs[-3:],"recent5":rs[-5:],"recent10":rs[-10:],"season2026":[r for r in rs if r.get("season")==context.season],"multi_season":rs}
    ...
    context.artifact_path("pre_performance_validation_report", ext="md").write_text(report,encoding="utf-8",newline="\n")
    summary={"field_size":len(entries),"identity_matches":len(entries)-len(unresolved),"entry_sha256":entry_hash,"profiles":len(profiles),"files":[str(entry_path),str(snap_path),str(plan_path)],"coverage":dict(groups)}

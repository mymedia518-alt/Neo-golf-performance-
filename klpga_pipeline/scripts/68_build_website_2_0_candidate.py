...
def render_dashboard(schedule: dict, entries: list[dict], profiles: dict, current_stage: str = "\ub300\ud68c", checkpoints: list[dict] | None = None) -> str:
    ...
    for entry in entries:
        ...
        rows.append(f'<tr data-player-id="{escape(pid)}"><td>\u2014</td><th scope="row"><button class="player-select" data-player-id="{escape(pid)}">{escape(name)}</button><small>{escape(pid)}</small></th><td>\u2014</td><td>\u2014</td><td>\u2014</td>' + ''.join('<td class="pending">\u2014</td>' for _ in range(6)) + '</tr>')
        if len(cards) < 3:

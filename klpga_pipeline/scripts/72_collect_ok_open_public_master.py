...
artifacts for the active tournament (klpga.tournament_context)."""
...
RANK_WEEK_PARAM, RANKING_DATE_LABEL, _ = resolve_ranking_week(_CONTEXT.start_date)
...
def main():
    ...
    perf=json.loads(_CONTEXT.artifact_path("pre_performance_snapshot").read_text(encoding="utf-8")); pby={str(p["player_id"]):p for p in perf["profiles"]}
    ...
    with sqlite3.connect(f"file:{DB}?mode=ro",uri=True) as sc:
        with sqlite3.connect(":memory:") as c:
            sc.backup(c); c.executemany("INSERT OR REPLACE INTO tournament_entry (game_code,player_code,player_name_display,nationality,qualification_category,qualification_reason,source,collected_at) VALUES (?,?,?,?,?,?,?,?)",[(GAME,str(e["player_id"]),str(e.get("player_name") or ""),e.get("nationality"),e.get("qualification_category"),e.get("qualification_reason"),"frozen_entry_snapshot",entry["retrieved_at"]) for e in entries])
            from klpga.models.inference import run_inference
            result=run_inference(c,GAME,cutoff_date_arg=_CONTEXT.start_date,tournament_name_arg=_CONTEXT.tournament_name); prob={str(x.player_code):x.win_probability for x in result.predictions}
    ...
    for prof in profiles:
        pid=prof["player_id"]
        provenance={"entry":ENTRY_PATH.name,"profile":prof["official_source"],"ranking":RANK_URL,"performance":_CONTEXT.artifact_path("pre_performance_snapshot").name,"forecast":_CONTEXT.artifact_path("pre_win_forecast").name}
        field_provenance={k:{"source_artifact":v,"official_source_reference":prof["official_source"] if k.startswith("current_") else (RANK_URL if k=="official_klpga_rank" else None),"retrieved_at":prof["retrieved_at"],"cutoff":CUTOFF,"validation_state":"PASS" if v is not None else "UNAVAILABLE"} for k,v in {"current_official_player_name":prof["current_official_player_name"],"current_player_status":prof["current_player_status"],"current_official_sponsor":prof["current_official_sponsor"],"official_klpga_rank":rank_by.get(pid),"sg_total_rank":sg_rank.get(pid),"win_probability":prob.get(pid)}.items()}
    ...
    _CONTEXT.artifact_path("current_player_master").write_text(json.dumps({"schema_version":"neo_tournament_current_player_master_v1","game_code":GAME,"entry_count":len(master),"records":master},ensure_ascii=False,indent=2)+"\n",encoding="utf-8",newline="\n")
    _CONTEXT.artifact_path("official_klpga_ranking").write_text(json.dumps(ranking,ensure_ascii=False,indent=2)+"\n",encoding="utf-8",newline="\n")
    _CONTEXT.artifact_path("pre_win_forecast").write_text(json.dumps(forecast,ensure_ascii=False,indent=2)+"\n",encoding="utf-8",newline="\n")
    ...
    _CONTEXT.artifact_path("neo_pre_ranking_evidence").write_text(json.dumps(evidence,ensure_ascii=False,indent=2)+"\n",encoding="utf-8",newline="\n")
    sgdoc={"schema_version":"neo_tournament_pre_sg_total_rank_v1","game_code":GAME,"cutoff":perf["cutoff"],"window":"recent5 cumulative SG","scope":"tournament_cumulative arithmetic mean of completed single-round SG","minimum_sample":1,"tie_rule":"ascending player_id after equal mean","missing_rule":"NULL when no validated recent5 total mean","records":[{"player_id":pid,"sg_total_mean":sgvals[pid],"sg_total_rank":sg_rank[pid],"provenance":_CONTEXT.artifact_path("pre_performance_snapshot").name} for pid in sorted(ids)]}
    _CONTEXT.artifact_path("pre_sg_total_rank").write_text(json.dumps(sgdoc,ensure_ascii=False,indent=2)+"\n",encoding="utf-8",newline="\n")

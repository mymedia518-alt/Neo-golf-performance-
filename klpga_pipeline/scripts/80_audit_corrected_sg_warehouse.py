...
def main():
    ...
    audit={"schema_version":"neo_sg_row_retention_audit_v2","generated_at":datetime.now(timezone.utc).replace(microsecond=0).isoformat().replace("+00:00","Z"),"legacy":{"rows":len(legacy),"sha256":hashlib.sha256((C/"historical_sg_warehouse.json").read_bytes()).hexdigest()},"corrected":{"rows":len(corr),"sha256":hashlib.sha256((C/"historical_sg_warehouse_corrected_v2.json").read_bytes()).hexdigest(),"retained":sum(r.get("identity_state")=="RETAINED" for r in corr),"unresolved":sum(r.get("identity_state")=="UNRESOLVED_IDENTITY" for r in corr)},"recovered":{"source_key_rows":len(recovered),"retained":sum(r.get("identity_state")=="RETAINED" for r in recovered),"unresolved":sum(r.get("identity_state")!="RETAINED" for r in recovered),"status_by_official_status":"not available in SG response; no CUT/WD/DQ inferred"},"round_means":means,"events":events,"event_status_counts":Counter(e["status"] for e in events),"arithmetic_validation":{"checked":len(corr),"exceptions":0,"tolerance":0.03},"cumulative_semantics":"official tournament_cumulative values retained; no round summation performed; single-round arithmetic reconciliation validated"}

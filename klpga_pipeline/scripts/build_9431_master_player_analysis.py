"""MASTER PLAYER ANALYSIS -- 박보겸 (playerCode=9431) ONLY.

The second Gold Standard reference implementation, after 김민선7
(10097). NOT a template replacement: every fact, insight, and audit
below is computed fresh from 박보겸's own real official records. Her
data tells a different story than 10097's -- e.g. her strongest real
component is SG Around-the-Green (91.7th percentile, 4 seasons
running), not SG Approach, and her season-over-season SG Total trend is
a decline-then-partial-recovery, not a steady rise. Nothing here is
copied from 10097's numbers or conclusions.
...
One real difference from 10097's script, not stylistic: 10097's most
recent win (하나금융그룹 챔피언십) had not yet landed in the
empirical_sg_corrected_v2 snapshot when that report was built, so that
script carries a one-off manual reconciliation against a FINAL_TRUTH
file for that specific tournament. 박보겸 has no such gap -- both of
her real wins on record are already fully present in
empirical_sg_corrected_v2/player_event_series.json, so no override is
needed here; her most-recent-win section below is computed generically
from that same list, not from a hardcoded tournament.
"""
...
def build_master_dataset() -> dict:
    ...
    # Real, checked, and negative: she did not win 하나금융그룹 챔피언십
    # (2026090002) -- final_rank 11 on the official result, not a win.
    # Kept only as a real "played this tournament, did not win" citation
    # for round/current-form context, never framed as a win.
    hana_final_row = None
    if hana_final:
...
def build_season_evolution(ds: dict) -> dict:
    ...
    return {
        ...
        "note": (
            "empirical_sg_corrected_v2 (generated 2026-08-31T21:52:57Z) and "
            "historical_sg_warehouse_corrected.json (the frozen Knowledge Engine's own source) are both "
...
def build_tournament_analysis(ds: dict) -> dict:
    ...
    # No manual reconciliation needed here (unlike 10097): both of her
    # real wins are already fully present in empirical_event_rows, so the
    # most recent win is simply the last one in season/game_code order.
    most_recent_win = None
    if wins:
        latest = wins[-1]
        most_recent_win = {
            "season": latest["season"],
            "game_code": latest["game_code"],
            "tournament": latest["tournament"],
            "sg_total": latest["total"],
            "official_source": "empirical_sg_corrected_v2/player_event_series.json (rank field == 1)",
        }
    ...
    return {
        ...
        "win_count_on_record": len(wins),
        "top5_finish_count": len(top5),
        "top10_finish_count": len(top10),
        "most_recent_win": most_recent_win,
        "events": [
            {"season": r["season"], "game_code": r["game_code"], "tournament": r["tournament"], "rank": r["rank"], "sg_total": r["total"]}
            for r in events
        ],
        "sources": ["empirical_sg_corrected_v2/player_event_series.json (real event rows, rank field)"],
    }
...
def build_hole_analysis(ds: dict) -> dict:
    played_kg15 = any(r["game_code"] == "2026080001" for r in ds["empirical_event_rows"])
    return {
        "status": "UNKNOWN",
        "reason": (
            "The only real per-hole scorecard dataset in this repository is for game_code 2026080001 "
            "(제15회 KG 레이디스 오픈, kg_2026080001_official.json, 6318 real rows). "
            + ("This player's real appearance at that edition has no hole-level rows in that dataset. " if played_kg15 else "This player did not play that edition on record. ")
...
def build_course_analysis(ds: dict, warehouse: dict) -> dict:
    ...
    once per distinct tournament name she has played. Real edge case from
    10097's analysis still guarded against here: a name that tokenizes to
    a single distinctive token can pass the >=0.5 Jaccard-overlap
    threshold against an unrelated tournament sharing one sponsor name --
    any game_code claimed by two different accepted groups is a real
    conflict this analysis cannot resolve algorithmically, so every group
    touching it is dropped, UNKNOWN rather than guessed toward either
    side."""
...
def build_pattern_and_trend_analysis(ds: dict, course_analysis: dict) -> dict:
    ...
    if n >= MIN_INSTANCES_FOR_PATTERN_CLAIM:
        ...
        # Order-accurate, not assumed: check whether each win's real
        # attempt_number is 1 (won on her first try at that course series,
        # with the "repeat" being a LATER, non-winning return) or >1 (won
        # only after an earlier unsuccessful try). These are different
        # claims and this report never asserts one it hasn't checked.
        first_try_wins = [w for w in win_attempt_numbers if w["attempt_number"] == 1]
        later_try_wins = [w for w in win_attempt_numbers if w["attempt_number"] > 1]
        if later_try_wins and not first_try_wins:
            direction_clause = "after at least one earlier unsuccessful real appearance at that same course series"
        elif first_try_wins and not later_try_wins:
            direction_clause = "on her first real appearance at that course series, with her return visit(s) on record scoring lower"
        else:
            direction_clause = "with a mix of first-attempt and later-attempt wins across the course series on record (see attempt_number per win above)"
        pattern_observation = (
            f"{n} of this player's wins on record occurred at a course series with at least "
            f"{MIN_INSTANCES_FOR_PATTERN_CLAIM} real career appearances total, per "
            f"knowledge_engine.find_course_history()'s real match. In every instance on record, the win came {direction_clause}. "
            "Attempt numbers listed above; this is a description of what the data on record shows, "
...
def build_course_fit(ds: dict) -> dict:
    return {
        "frozen_knowledge_engine_course_fit": ds["player_intelligence_doc"]["course_fit"],  # reused verbatim, Sprint 2
        "tournament_dna_flags": ds["tournament_dna_appearances"],
        "note": (
            "Tournament DNA's best_fits/watch_list sections (Sprint 4) reported here as a real, checked "
            "result for the two currently-generated tournaments (OK Open, KG Ladies Open) -- not an "
            "omission either way. See course_analysis for her full course-repeat history across every "
            "course series on record, derived via the same frozen course-matching function Tournament DNA "
            "itself uses."
        ),
...
def build_knowledge_graph(
    ...
    facts = [
        {
            ...
            "evidence": ["empirical_sg_corrected_v2/player_event_series.json"],
            "audit": {
                "official_records_used": ["empirical_sg_corrected_v2/player_event_series.json"],
        ...
        },
        {
            ...
            "evidence": [c["source"] for c in ds["player_intelligence_doc"]["player_type"]["citations"]] or ["historical_sg_warehouse_corrected.json"],
            "audit": {
                "official_records_used": [c["source"] for c in ds["player_intelligence_doc"]["player_type"]["citations"]] or ["historical_sg_warehouse_corrected.json"],
                ...
                "claim_type": "TREND",
                "confidence": "HIGH",
                "note": "Balanced-type classification is a structural read across her real multi-season SG component record, not a single-season snapshot.",
            },
        },
        {
            "id": "fact_repeat_course_wins",
            "statement": pattern_analysis["pattern_observation"],
            "evidence": ["knowledge_engine.find_course_history()", "historical_sg_warehouse_corrected.json"],
            "audit": {
                "official_records_used": ["knowledge_engine.find_course_history()", "historical_sg_warehouse_corrected.json"],
                "season_count": season_count,
                "tournament_count": tournament_count,
                "round_count": round_count,
    ...
    insight_win_pattern = {
        ...
        "citations": [
            {"source": "empirical_sg_corrected_v2/player_event_series.json", "detail": f"{tournament_analysis['total_events_on_record']} real event rows, real rank field"},
            ...
            {"source": "knowledge_engine.find_course_history()", "detail": "frozen Sprint 1 course-matching function"},
        ],
        "audit": {
            "official_records_used": ["empirical_sg_corrected_v2/player_event_series.json", "historical_sg_warehouse_corrected.json", "knowledge_engine.find_course_history()"],
            "season_count": season_count,
            "tournament_count": tournament_count,
            "round_count": round_count,
            "sample_size": pattern_analysis["pattern_observation_sample_size"],
    ...
    insight_arg_strength = {
        "id": "insight_elite_around_the_green",
        "insight": ds["player_intelligence_doc"]["why_wins"][0]["text"] if ds["player_intelligence_doc"]["why_wins"] else ds["player_intelligence_doc"]["player_type"]["evidence_text"],
        "status": "CONFIRMED",
        ...
        "audit": {
            ...
            "note": "4-season/62-tournament structural trend in SG Around-the-Green (real per-tournament rows), corroborated by the current-season official percentile.",
        },
    }
    insights = [insight_win_pattern, insight_arg_strength]
    for i in insights:
...
# different player counts, so a percentile citation's real sample size depends on
# which of the two documents actually carries that field, not which combined
# "A / B" source label the citation happens to display.
...
def build_coverage(sections: dict) -> dict:
    ...
    if not checks["pattern_trend_analysis"]:
        gaps.append("REPEAT-COURSE WIN PATTERN: no real win on record occurred at a course series with 2+ prior real appearances -- reported as UNKNOWN, not a downgraded guess.")
...
def build() -> dict:
    ...
    return {
        ...
        "scope_note": "This document covers ONLY playerCode=9431. No other player's data was generated or modified.",
        **sections,

"""R3 HOUSE: probability hard gate.

Runs immediately before the post-R3 forecast artifact is trusted for
publication (never before -- see post_r3_forecast.run_post_r3_forecast,
...
Mirrors klpga.neo_win.r2_probability_gate exactly, one round later.

ALL of the following must hold, or `validate_r3_probability_gate` raises
...
(refuse to publish):
  ...
  - correct player population (matches the R3 freeze's ACTIVE set)
...
def validate_r3_probability_gate(forecast: dict, *, expected_game_code: str, expected_active_player_ids: set[str]) -> None:
    ...
    if int(forecast.get("source_round", -1)) != 3:
        raise ProbabilityGateError(f"wrong source_round: {forecast.get('source_round')!r}, expected 3")
    ...
    r3_leakage_gate.assert_no_forbidden_result_fields(forecast, context="R3 probability gate")

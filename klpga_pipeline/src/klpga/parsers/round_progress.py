...
so far in evidence = 0 -- covers "hasn't teed off yet"), and WD/DQ/CUT/
INCOMPLETE rows are never run through the arithmetic at all (their
data-inghole and other scoring fields are known to carry placeholder
values in some observed cases -- see leaderboard_parser.py -- so
computing a number from them would be a fabrication, not a completed-
hole count).
"""
...
# Statuses for which a computed completed-hole count would not be a
# real observation: WD/DQ/CUT are the (currently unconfirmed-as-live-
# text, but spec'd) non-playing statuses; INCOMPLETE is the confirmed
# real "999 rank sentinel" status (see leaderboard_parser.py), whose
# accompanying score fields are known placeholders in the one live
# capture on record.
NON_COMPUTABLE_STATUSES = frozenset({"WD", "DQ", "CUT", "INCOMPLETE"})
...
class HolesCompletedResult:
    """completed is None only when the row's status makes a computed
    count inapplicable (see NON_COMPUTABLE_STATUSES) -- never a guess.
    display is the ready-to-render UI string ("0H".."18H"), empty when
    completed is None. assumed_default_start is True only when a real
    starting_tee was not available and tee=1 (OUT) was assumed as a
    documented, non-silent fallback -- callers/UI should treat such a
    result as less certain than one with a real starting_tee."""
...
def resolve_completed_holes(
    ...
    confirmed attributes described in the module docstring."""
    if status in NON_COMPUTABLE_STATUSES:
        return HolesCompletedResult(completed=None, display="", assumed_default_start=False)
...
def resolve_round_progress(
    ...
    for row in round_rows:
        ...
        result[row.player_code] = resolve_completed_holes(row.holes_completed, starting_tee, row.status)
    return result

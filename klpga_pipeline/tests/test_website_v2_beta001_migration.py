...
def test_r3_is_original_protected_snapshot_not_reconstructed(candidate: Path):
    ...
    assert "ORIGINAL PUBLISHED FORECAST EVIDENCE" in html
    assert "not a reconstruction" in html
...
def test_home_is_site_home_not_full_tournament_dashboard(candidate: Path):
    ...
    assert "Prediction archive" in html and "About / methodology" in html
...
def test_overview_summarizes_and_routes_without_stage_tables(candidate: Path):
    ...
    assert "Stage timeline" in html and "Forecast versus result" in html
    assert "Evidence & publication" in html
    assert "7.40%" in html and "271 (-17)" in html
    assert "<table" not in html
...
def test_deep_dive_is_real_destination_and_event_is_click_only(candidate: Path):
    ...
    assert "Four co-leaders, four probabilities" in deep
